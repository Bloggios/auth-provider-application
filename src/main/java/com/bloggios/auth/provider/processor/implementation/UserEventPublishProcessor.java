...
import com.bloggios.auth.provider.payload.event.UserEvent;
...
@Component
public class UserEventPublishProcessor {

    private final ModelMapper modelMapper;
    private final UserEventKafkaProducer userEventKafkaProducer;

    public UserEventPublishProcessor(
            ModelMapper modelMapper,
            UserEventKafkaProducer userEventKafkaProducer
    ) {
        this.modelMapper = modelMapper;
        this.userEventKafkaProducer = userEventKafkaProducer;
    }

    public void process(UserEntity userEntity) {
        UserEvent userEvent = modelMapper.map(userEntity, UserEvent.class);
        userEvent.setEventType(EventType.CREATE);
        userEventKafkaProducer.sendMessage(userEvent);
    }
}
