...
public class EmailValidationProvider implements BusinessValidator<String> {
    ...
    public EmailValidationProvider(
    ...
    public void validate(String email) {
        if (ObjectUtils.isEmpty(email))
            throw new BadRequestException(ExceptionCodes.EMAIL_MANDATORY);
        if (!email.matches(ServiceConstants.EMAIL_REGEX)) {
            throw new BadRequestException(ExceptionCodes.EMAIL_NOT_VALID);
        }
        if (userEntityDao.existsByEmail(email)) {
            throw new BadRequestException(ExceptionCodes.EMAIL_ALREADY_PRESENT);
        }
    }
}
