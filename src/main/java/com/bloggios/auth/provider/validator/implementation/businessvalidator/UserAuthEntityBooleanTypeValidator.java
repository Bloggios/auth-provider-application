...
@Component
public class UserAuthEntityBooleanTypeValidator implements BusinessValidator<UserEntity> {

    @Override
    public void validate(UserEntity userEntity) {
        if (!userEntity.isEnabled()) throw new BadRequestException(ExceptionCodes.USER_INACTIVE);
        if (!userEntity.isAccountNonExpired()) throw new BadRequestException(ExceptionCodes.ACCOUNT_EXPIRED);
        if (!userEntity.isCredentialsNonExpired()) throw new BadRequestException(ExceptionCodes.USER_CREDENTIALS_EXPIRED);
        if (!userEntity.isAccountNonLocked()) throw new BadRequestException(ExceptionCodes.ACCOUNT_LOCKED);
    }
}
