...
@Component
public class ForgetPasswordUserAuthExhibitor implements Exhibitor<UserEntity> {

    private final UserAuthEntityBooleanTypeValidator userAuthEntityBooleanTypeValidator;

    public ForgetPasswordUserAuthExhibitor(
            UserAuthEntityBooleanTypeValidator userAuthEntityBooleanTypeValidator
    ) {
        this.userAuthEntityBooleanTypeValidator = userAuthEntityBooleanTypeValidator;
    }

    @Override
    public void validate(UserEntity userEntity) {
        userAuthEntityBooleanTypeValidator.validate(userEntity);
        if (!userEntity.getProvider().equals(Provider.email)) {
            throw new BadRequestException(ExceptionCodes.PROVIDER_SHOULD_EMAIL_FOR_FORGET_PASSWORD);
        }
    }
}
